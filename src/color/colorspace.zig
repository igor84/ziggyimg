...
fn linearToHybridGamma(comptime breakPoint: comptime_float, comptime linearFactor: comptime_float,
                       comptime fac: comptime_float, comptime exp: comptime_float) fn (f32) f32 {
    return struct {
        fn impl(v: f32) f32 {
...
fn hybridGammaToLinear(comptime breakPoint: comptime_float, comptime linearFactor: comptime_float,
                       comptime fac: comptime_float, comptime exp: comptime_float) fn (f32) f32 {
    return struct {
        fn impl(v: f32) f32 {
...
pub const rgbColorSpaces = [_]*const RGBColorSpace {
    &sRGB, &ntsc1953, &ntsc, &ntsc_j, &pal_secam, &rec709, &rec2020,
    &adobeRGB, &wideGamutRGB, &appleRGB, &proPhoto, &cieRGB,
    &p3dci, &p3d65, &p3d60, &displayP3
};
